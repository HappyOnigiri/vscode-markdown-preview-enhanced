...
- **New commands "Open Current Source in Editor" and "Open Current Source in Editor and Close Preview"** — Open the Markdown file the focused preview is rendering in a text editor, either alongside the preview or replacing it. This is the way back to the source after following links inside the preview, where the preview shows `subdir/c.md` but the tab it was opened from still refers to `a.md`; it also gives `"previewMode": "Previews Only"` an exit into the editor, since the preview occupies the tab there. The editor opens in the preview's own editor group, and the preview tab is only closed once the editor has opened, so a failure to open leaves the preview in place.

### Performance

All of the following ship with [crossnote 0.9.34](https://github.com/shd101wyy/crossnote/releases/tag/0.9.34).
...
- **"Open In-preview Editor" works offline** — right-clicking the preview and choosing _Edit Markdown → Open In-preview Editor_ mounted the editor shell but loaded the monaco editor itself at runtime from `cdn.jsdelivr.net`, so offline or firewalled setups sat on "Loading editor..." forever and the menu item looked like it did nothing ([#2164](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2164) reported by @CVStratACGrant). The editor is now bundled locally (monaco-editor upgraded 0.43.0 → 0.55.1) and opens instantly with zero network requests ([crossnote 0.9.34](https://github.com/shd101wyy/crossnote/releases/tag/0.9.34)).
- **"Open In-preview Editor" no longer silently does nothing** — the menu item attached the editor to the first `.final-line` element on the page and gave up when none was visibly usable: while a render was in flight, the _hidden_ preview copy also held a `.final-line` element, so a click there portaled the editor into an invisible element that the next render deleted, and in zen mode (or a file with no source map) no anchor existed at all. The lookup is now scoped to the real preview container, and when no anchor exists a fresh one is appended at the end of the preview so the editor always opens; the menu item is hidden in zen mode, where the in-preview editor cannot be shown ([crossnote 0.9.34](https://github.com/shd101wyy/crossnote/releases/tag/0.9.34)).
- **Fix the preview's file watcher deleting a freshly recreated Markdown file (data loss)** — when Git removed and then recreated a Markdown file during a branch checkout, the watcher's delayed deletion notification could delete the freshly recreated replacement, surfacing as unstaged deletions in `git status`. The underlying `deleteNote(path, alreadyDeleted)` call had its flag wired backwards: `true` — which the watcher passes for every deletion event — guaranteed an `unlink()` ran instead of skipping it. The watcher no longer deletes a file that exists at the path; internal bookkeeping still runs when a note really is gone ([crossnote#481](https://github.com/shd101wyy/crossnote/issues/481) reported by @Josh-Cena; fixed in [crossnote 0.9.34](https://github.com/shd101wyy/crossnote/releases/tag/0.9.34)).
...
- **Offline HTML export produces a portable single file** — "Export HTML (offline)" used to emit `file://` `<link>`/`<script src>` tags pointing at the local `crossnote/dependencies/` directory (KaTeX CSS, Mermaid, WaveDrom, Vega, Reveal, Font Awesome), which break as soon as the file is copied to another machine. Those assets are now inlined into the document, with CSS font references rewritten to `data:` URIs (woff2 preferred). Open in Browser / Chrome PDF / Prince keep using local files so temporary documents are not bloated with a ~3.5MB mermaid bundle, and CDN export is unchanged ([crossnote 0.9.33](https://github.com/shd101wyy/crossnote/releases/tag/0.9.33)).
- **New `markdown-preview-enhanced.frontMatterRenderingOption` value: `"vertical table"`** — renders front matter as a table with one key and value per row instead of one column per key, so the table stays inside the preview pane when there are many keys or long values (the layout VS Code's built-in preview uses). The setting also gained localized descriptions for all four values ([#2371](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2371) reported by @JaeyeongYang; renders via [crossnote 0.9.33](https://github.com/shd101wyy/crossnote/releases/tag/0.9.33)).

### Security

- **Stop the note index from scanning outside the workspace** — when the resolved notebook root was the filesystem root itself (VS Code opened on `/` or a Windows drive root, a standalone Markdown file directly under it, or an untitled document whose fallback root `/.` resolves to `/`), building the wikilink/backlink/tag/graph index recursively stat'ed and read files across the whole machine; a symbolic link inside the workspace could similarly let the walk escape it. crossnote now refuses to index a filesystem-root notebook and never follows symbolic links during the walk (link entries are not indexed), and permission errors (`EACCES`/`EPERM`) encountered while walking are no longer logged as errors per directory. The extension shows a one-time warning when the notebook root is a filesystem root, since wikilinks/backlinks/graph will find nothing until a real folder is opened ([#2376](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2376) reported by @prawnsalad; ships with [crossnote 0.9.33](https://github.com/shd101wyy/crossnote/releases/tag/0.9.33)).
- **Unsaved (`untitled:`) Markdown buffers now index against the current workspace** — the notebook root for a non-file document used to resolve to the `/.` spelling of the filesystem root, the exact path that let clicking backlinks on an unsaved buffer trigger the whole-machine scan in [#2376](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2376) (confirmed by the reporter). It is now the first workspace folder, so backlinks, wikilink completions and the graph view work on scratch buffers too; with no folder open, indexing is refused as before.
- **Validate the imgur upload endpoint override** — the `IMGUR_API_URL` environment override was sent uploads (and the client id) without validation; it is now required to be an http(s) URL at a public host — localhost, `*.local`/`*.internal`, private, link-local (including cloud metadata addresses), and reserved ranges are rejected with a clear error ([crossnote 0.9.33](https://github.com/shd101wyy/crossnote/releases/tag/0.9.33)).

### Bug fixes

- **Fix preview crash (`TypeError: Cannot read properties of null (reading 'length')`) on callouts with a blank line under the marker** — an Obsidian-style callout whose `[!NOTE]` marker line stands alone or is followed by a blank quoted line crashed the entire preview; 0.8.32 introduced it via a crossnote callout change ([#2375](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2375) reported by @GodExious; fixed in [crossnote 0.9.33](https://github.com/shd101wyy/crossnote/releases/tag/0.9.33)).
- **Fix footnotes rendered inside a callout title** — an empty copy of the document's footnotes section appeared inside the callout title in addition to the real one at the bottom; footnotes now appear exactly once ([#2377](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2377) reported by @chz-cn; fixed in [crossnote 0.9.33](https://github.com/shd101wyy/crossnote/releases/tag/0.9.33)).
- **Fix `{highlight=...}` bands rendering below the code block in most code themes, and covering code text with the `vscode` theme** — only a couple of code themes carried the structural CSS for the highlight band, so it rendered in flow below the code; the structure now lives in the shared stylesheet, themes override just the band color, and with the `vscode` theme the band is blended to 35% opacity instead of painting VS Code's opaque selection color over the code ([#2379](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2379) and [#2378](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2378) reported by @chz-cn; fixed in [crossnote 0.9.33](https://github.com/shd101wyy/crossnote/releases/tag/0.9.33)).
...
- **Fix slow rendering of documents with long lines** — crossnote's `^block-id` transform's cost grew quadratically with line length, so large documents written one sentence per line could take seconds per preview refresh. It now runs in linear time, including on lines ending in long whitespace runs. ([crossnote#455](https://github.com/shd101wyy/crossnote/pull/455) by @zeyutang)
...
- **Stop forcing the preview tab to the front when switching markdown files (Multiple Previews)** — In Multiple Previews mode, activating a markdown file that already had a preview open would always reveal that preview tab to the front of its group, disrupting the editor layout on every file switch — even though focus was preserved. This auto-reveal is now gated behind the existing `automaticallyShowPreviewOfMarkdownBeingEdited` setting (default `false`), matching that setting's intent: leave it off to keep your layout, or turn it on to have the relevant preview surface as you edit. Fixes [#2286](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/issues/2286). Reported by @RandellTsen.
